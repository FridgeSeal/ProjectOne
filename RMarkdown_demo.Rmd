...
ImpDatePlot = BasePlot +
  geom_path(aes(y = Impressions), size = 0.4, colour = "#1DABB8") +
  ...
  theme_few() +
  scale_colour_few() +
  theme(plot.title = element_text(size = 16, family = "Ubuntu"),text = element_text(family = "Ubuntu Light")) +
...
CTRHourly = ggplot(HourData, aes(x = HourData$Hour)) +
  geom_path(aes(y = HourData$CTR)) +
  geom_area(aes(y = HourData$CTR), fill = "#009BDC", alpha = 0.5) +
...
CTRCirclePlot = ggplot(HourData, aes(x=HourData$Hour)) +
  geom_bar(aes(y = CTR, fill = Hour), width = 1, stat = "identity") +
  scale_x_continuous(breaks = seq(0,23, by =2)) +
  theme_update(axis.text = element_blank(), axis.ticks.y = element_blank()) +
  ...
  theme(panel.border = element_blank()) + coord_polar(start = -0.1) +
...
OSData = ggplot(OStemp, aes(fill = OS)) +
 geom_rect(aes(ymax = OStemp$ImpMax, ymin = OStemp$ImpMin, xmax = 0.6, xmin = 0.3), colour = "white") +
 geom_rect(aes(ymax = OStemp$ClickMax, ymin = OStemp$ClickMin, xmax = 0.3, xmin = 0), colour = "white") +
 ...
 xlim(-0.1,0.7) +
 ylim(-0.2,1) +
 theme(panel.border = element_blank(), axis.ticks = element_blank(), axis.text = element_blank(), legend.position = "bottom") +
...
Stateplot = ggplot(Statetemp, aes(fill = State)) +
  ...
  labs(x = "", y = "") + theme(panel.grid=element_blank()) + theme(axis.text=element_blank()) +
  theme(axis.ticks=element_blank()) +
  theme(panel.background = element_blank()) +
  theme(legend.title = element_blank()) +
  guides(fill = guide_legend(override.aes = list(colour = NULL))) + theme(plot.margin = unit(c(0,0,0,0),"cm"))
...
```{r, echo = FALSE, collapse = TRUE, warning= FALSE, message= FALSE, fig.show='hold', fig.asp=0.5, fig.align='center', CairoPlot, dev='CairoPNG'}
OSData
```
