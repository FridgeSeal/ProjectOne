...
CTRCirclePlot = ggplot(HourData, aes(x=HourData$Hour)) + geom_bar(aes(y = CTR, fill = Hour), width = 1, stat = "identity")+ scale_x_continuous(breaks = seq(0,23, by =2)) + theme_update(axis.text = element_blank(), axis.ticks.y = element_blank()) + theme_few() + scale_colour_few() + theme(plot.margin = unit(c(0,0,0,0),"cm")) + theme(axis.title = element_blank()) + theme(plot.title = element_text(size = 16, family = "Ubuntu"),text = element_text(family = "Ubuntu Light")) + guides(fill = FALSE) + labs(title = "CTR By Hour", x = NULL, y = NULL) +theme(panel.border = element_blank()) + coord_polar(start = -0.1) + theme(axis.text.y = element_blank())
