...
OSImps = ggplot(OStemp, aes(fill = OS)) + geom_rect(aes(ymax = OStemp$ImpMax, ymin = OStemp$ImpMin, xmax = 0.3, xmin = 0), colour = "white") + geom_rect(aes(ymax = OStemp$ClickMax, ymin = OStemp$ClickMin, xmax = 0.6, xmin = 0.3), colour = "white") + theme(panel.grid=element_blank()) + theme(axis.text=element_blank()) + theme(axis.ticks=element_blank()) + theme(panel.background = element_blank()) + theme(legend.position = "bottom") + theme(legend.title = element_blank()) + labs(x= NULL, y = NULL) + theme(plot.margin = unit(c(0,0,0,0),"cm")) + geom_text(aes(label = paste(round(ImpFrac*100, 2), "%"), x = 0.15, y = ((ImpMax + ImpMin)/2), inherit.aes = TRUE, show.legend = FALSE), family = "Ubuntu Light") + geom_text(aes(label = paste(round(ClickFrac*100,2),"%"), x = 0.45, y = ((ClickMin + ClickMax)/2), inherit.aes = TRUE, show.legend = FALSE)) +coord_flip() + theme(panel.border = element_blank(), axis.ticks = element_blank()), axis.text = element_blank() + theme_few() + scale_colour_few()
# OSClicks = ggplot(OStemp, aes(fill = OS)) + geom_rect(aes(ymax = OStemp$ClickMax, ymin = OStemp$ClickMin, xmax = 2.7, xmin = 2.3)) + xlim(0,2.8) + annotate("text", x = 0, y = 0, label = "Clicks by OS", size = 10, fontface = "bold") + theme(panel.grid=element_blank()) + theme(axis.text=element_blank()) + theme(axis.ticks=element_blank()) + theme(panel.background = element_blank()) + theme(legend.position = "bottom") + theme(legend.title = element_blank()) + labs(x= "", y = "") + theme(plot.margin = unit(c(0,0,0,0),"cm"))
